...
def _poll_terminal_job(
    ...
    *,
    ...
    cancel_status, cancel_payload = _request(
        ...
        json={"reason": "Round 2 timeout guard reached before terminal state."},
    )
...
def _replacement_pairs(
    ...
    *,
    ...
    root_placeholder = f"scratch://round-2/{paths.run_id}"
...
def _build_report(
    *,
    ...
    lines: list[str] = []
    lines.append("# Recipe Agent v1 Round 2 Report")
    lines.append("")
    ...
    lines.append(f"- **Retained external evidence root:** `scratch://round-2/{run_paths.run_id}`")
    lines.append("")
    ...
    lines.append("- Paths represented in committed artifacts by `scratch://round-2/<run_id>/...` placeholders.")
    lines.append("")
    return "\n".join(lines)


def main() -> int:
    parser = argparse.ArgumentParser(description="Run Recipe Agent v1 Round 2 baseline over frozen models.")
    parser.add_argument(
        "--scratch-root",
        default=r"C:\fmo-recipe-v1\round-2",
    ...
    parser.add_argument(
        "--output-dir",
        default="",
        help="Optional output directory. Defaults to evaluation/recipe-agent-v1/round-2.",
    )
    ...
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("r2-%Y%m%dT%H%M%SZ")
    ...
    manifest_payload: dict[str, Any] = {
        ...
        "scratch_locations": {
            "root": f"scratch://round-2/{paths.run_id}",
            "state": f"scratch://round-2/{paths.run_id}/state",
            "workspace": f"scratch://round-2/{paths.run_id}/workspace",
            "cache": f"scratch://round-2/{paths.run_id}/cache",
            "service_db": f"scratch://round-2/{paths.run_id}/state/service.sqlite3",
            "recipe_attempt_db": f"scratch://round-2/{paths.run_id}/state/recipe-attempts.sqlite3",
        },
    ...
    _write_json(paths.output_root / "round-2-summary.json", _sanitize_obj(summary_payload, replacements))
    ...
    (paths.output_root / "round-2-report.md").write_text(report, encoding="utf-8")
    return 0
