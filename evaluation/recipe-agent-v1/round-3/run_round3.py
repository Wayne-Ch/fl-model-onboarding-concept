...
def _poll_terminal_job(
    ...
    *,
    ...
    cancel_status, cancel_payload = _request(
        ...
        json={"reason": "Round 3 timeout guard reached before terminal state."},
    )
...
def _load_round2_snapshot_index(summary_path: Path) -> dict[tuple[str, str], str]:
    ...
    for row in results:
        ...
        revision_sha = str(row.get("manifest_sha", "")).strip().lower()
        job_id = str(row.get("job_id", "")).strip()
        if not model_id or not job_id:
            continue
        if re.fullmatch(r"[0-9a-f]{40}", revision_sha) is None:
            continue
        index[(model_id, revision_sha)] = job_id
    return index
...
def _seed_round2_snapshots(
    *,
    paths: RunPaths,
    models: list[dict[str, Any]],
    round2_runtime_root: Path,
    round2_summary_path: Path,
) -> list[dict[str, Any]]:
    index = _load_round2_snapshot_index(round2_summary_path)
    ...
    for row in models:
        ...
        status = "missing-round2-source"
        source_job_id = index.get((model_id, revision_sha))
        if target_dir.exists():
            status = "already-present"
        elif source_job_id is not None:
            source_dir = round2_runtime_root / "workspace" / source_job_id / "snapshot"
            if source_dir.is_dir() and (source_dir / "config.json").is_file():
                try:
...
def _replacement_pairs(
    ...
    *,
    ...
    root_placeholder = f"scratch://round-3/{paths.run_id}"
...
def _build_report(
    *,
    ...
    snapshot_reuse: list[dict[str, Any]],
) -> str:
    ...
    lines: list[str] = []
    lines.append("# Recipe Agent v1 Round 3 Report")
    lines.append("")
    ...
    lines.append(f"- **Retained external evidence root:** `scratch://round-3/{run_paths.run_id}`")
    lines.append("")
    ...
    if snapshot_reuse:
        lines.append("- Round 2 snapshot reuse seed status:")
        for row in snapshot_reuse:
    ...
    lines.append("- Paths represented in committed artifacts by `scratch://round-3/<run_id>/...` placeholders.")
    lines.append("")
    return "\n".join(lines)


def main() -> int:
    parser = argparse.ArgumentParser(description="Run Recipe Agent v1 Round 3 baseline over frozen models.")
    parser.add_argument(
        "--scratch-root",
        default=r"C:\fmo-recipe-v1\round-3",
    ...
    parser.add_argument(
        "--output-dir",
        default="",
        help="Optional output directory. Defaults to evaluation/recipe-agent-v1/round-3.",
    )
    parser.add_argument(
        "--reuse-round2-runtime-root",
        default="",
        help="Optional Round 2 runtime root for snapshot cache seeding.",
    )
    parser.add_argument(
        "--reuse-round2-summary",
        default="",
        help="Optional Round 2 summary JSON path for snapshot cache seeding.",
    )
    ...
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("r3-%Y%m%dT%H%M%SZ")
    round2_runtime_root = (
        Path(args.reuse_round2_runtime_root).resolve()
        if args.reuse_round2_runtime_root.strip()
        else Path(r"C:\fmo-r2\r2-20260902t1950z")
    )
    round2_summary_path = (
        Path(args.reuse_round2_summary).resolve()
        if args.reuse_round2_summary.strip()
        else (script_root.parent / "round-2" / "round-2-summary.json").resolve()
    )
    ...
    snapshot_reuse = _seed_round2_snapshots(
        paths=paths,
        models=[row for row in models if isinstance(row, dict)],
        round2_runtime_root=round2_runtime_root,
        round2_summary_path=round2_summary_path,
    )
    ...
    summary_payload: dict[str, Any] = {
        ...
        "round2_snapshot_reuse": snapshot_reuse,
    }
    ...
    manifest_payload: dict[str, Any] = {
        ...
        "scratch_locations": {
            "root": f"scratch://round-3/{paths.run_id}",
            "state": f"scratch://round-3/{paths.run_id}/state",
            "workspace": f"scratch://round-3/{paths.run_id}/workspace",
            "cache": f"scratch://round-3/{paths.run_id}/cache",
            "service_db": f"scratch://round-3/{paths.run_id}/state/service.sqlite3",
            "recipe_attempt_db": f"scratch://round-3/{paths.run_id}/state/recipe-attempts.sqlite3",
        },
    ...
    _write_json(paths.output_root / "round-3-summary.json", _sanitize_obj(summary_payload, replacements))
    ...
    report = _build_report(
        ...
        snapshot_reuse=_sanitize_obj(snapshot_reuse, replacements),
    )
    (paths.output_root / "round-3-report.md").write_text(report, encoding="utf-8")
    return 0
