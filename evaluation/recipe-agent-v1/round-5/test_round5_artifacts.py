...
def test_round5_summary_covers_frozen_five_and_valid_baseline() -> None:
    summary = _read_json(ROUND_DIR / "round-5-summary.json")
...
def test_round5_summary_contains_quality_evidence_fields() -> None:
    summary = _read_json(ROUND_DIR / "round-5-summary.json")
    results = summary["results"]  # type: ignore[index]
    assert isinstance(results, list)
    for row in results:
        assert "quality_validation_evidence_status" in row
        assert "quality_validation_metrics_ref" in row
        assert "quality_validation_baseline_comparison" in row
        assert "quality_validation_failure_excerpt" in row


def test_round5_manifest_records_ready_environment_probe() -> None:
...
def test_round5_artifacts_do_not_expose_raw_windows_absolute_paths() -> None:
    manifest = _read_json(ROUND_DIR / "round-manifest.json")
    summary = _read_json(ROUND_DIR / "round-5-summary.json")
...
def test_round5_per_model_artifacts_exist_for_all_five() -> None:
